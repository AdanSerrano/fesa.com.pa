...
interface CategoryColumnActions {
  onOpenDialog: (dialog: AdminProductsDialogType, category: ProductCategory) => void;
...
interface ItemColumnActions {
  onOpenDialog: (dialog: AdminProductsDialogType, item: ProductItem) => void;
  onToggleStatus: (id: string, isActive: boolean) => void;
  translations: ColumnTranslations;
  locale: string;
}
...
const PriceCell = memo(function PriceCell({
  ...
  return <span className="font-medium">{formatter.format(price)}</span>;
});
...
): CustomColumnDef<ProductCategory>[] {
...
export function createItemColumns(
  ...
  return [
    ...
    },
    {
      ...
      enableSorting: true,
      align: "right",
      cell: ({ row }) => <PriceCell price={row.price} locale={locale} />,
    },
    {
      id: "sku",
      accessorKey: "sku",
