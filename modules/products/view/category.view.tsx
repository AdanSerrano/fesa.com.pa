...
import { getCategoryWithItemsAction } from "../actions/products.actions";
...
import { Home, Package, FolderOpen } from "lucide-react";
        ...
        <AnimatedSection animation="fade-down" delay={0}>
          <Breadcrumb>
            <BreadcrumbList>
              ...
              <BreadcrumbItem>
                <BreadcrumbLink asChild>
                  <Link href={`/${locale}/products`}>{t("title")}</Link>
                </BreadcrumbLink>
              </BreadcrumbItem>
              <BreadcrumbSeparator />
              <BreadcrumbItem>
          ...
          {category.items.length > 0 ? (
            <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
              {category.items.map((product, index) => (
                <AnimatedSection key={product.id} animation="fade-up" delay={120 + index * 30}>
                  <ProductCard
                    product={product}
                    locale={locale}
            ...
            <Empty>
              ...
              <EmptyHeader>
                <EmptyTitle>{t("noProductsInCategory")}</EmptyTitle>
