...
export function usePrefetch<T, P extends Record<string, unknown>>({
  ...
  const isValidCache = useCallback((cache: PrefetchCache<T>): boolean => {
    return Date.now() - cache.timestamp < cacheDuration;
  }, [cacheDuration]);

  // Get cached data if available and valid
  const getCached = useCallback((params: P): T | null => {
  ...
  const prefetch = useCallback(async (params: P): Promise<void> => {
    if (!enabled) return;
  ...
  const prefetchMany = useCallback(async (paramsArray: P[]): Promise<void> => {
...
export function usePaginationPrefetch<T, P extends { page: number }>({
  ...
  const getCachedPage = useCallback((page: number): T | null => {
